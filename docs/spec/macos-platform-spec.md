...
默认仍使用系统 UI 字体与 macOS 等宽字体；不捆绑第三方字体。界面字号缩放必须通过 UI token 统一生效，避免在视图里新增裸 `text_size(px(...))`。

默认下载目录仅作为默认目标体验：单/批量下载交互一致——设置了有效默认目录时先弹应用内确认 sheet（单文件：「使用默认目录/另存为…/取消」，另存为面板以默认目录为初始目录；批量：「使用默认目录/另选目录/取消」，另选受 GPUI 0.2.2 `prompt_for_paths` 无初始目录参数限制）；未设置默认目录时直接打开面板。均不自动回写设置。
