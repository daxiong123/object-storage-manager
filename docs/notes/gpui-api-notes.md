...
**预览必须复刻真实的父链**：本次在预览里给控件套了个定宽 `div`，于是控件被撑满，
而真实工具栏里控件是**内容尺寸 flex 行**的子项——结果输入框（`flex_1`，flex-basis 0）
在生产里塌成一条 18px 的缝，预览却显示正常。改法：预览照抄调用点的父容器
（同样的 `h_flex().w(...).items_center()`），并**先做反向验证**——把修复去掉，
确认预览能复现那个塌陷，才算这个预览有验收能力。
