...
| 标题栏内的交互控件必须拦 click 冒泡 | 统一标题栏（gpui-component `TitleBar`）在 macOS 上把**双击**当窗口缩放（`on_double_click → window.titlebar_double_click()`，而 `on_double_click` 只是 `on_click` 加 `click_count == 2` 过滤）。文本输入框依赖**双击选词**，事件冒泡上去就会顺手把窗口缩放掉——表现为「输入框里的文字选不中」。只拦 `on_mouse_down` **不够**：它只在 bubble 阶段生效，而 Input 自己的 mouse_down 在更深一层、且 Input 全模块都不拦 click（只有 `on_scroll_wheel` 里有 `stop_propagation`），click 会原样冒到 TitleBar。**做法**：给交互控件（或其紧邻容器，用 `.on_click(stop_click)`，见 `workspace/titlebar.rs` 的 `stop_click`）拦掉 click。**优先加在容器上而不是 Button 上**——容器在冒泡里晚于所有后代，不会挤掉按钮自己的 `on_click`；加到 Button 上有让按钮失灵的风险。**已知未覆盖**：标题栏左侧的侧栏/前进/后退按钮与面包屑分段（同样会双击缩放窗口），修法同上但要逐个确认按钮 click 的注册顺序。窗口空白处的双击缩放**故意保留**（macOS 惯例），所以不要把 stop 加到铺满整个标题栏的那层容器上。 |
