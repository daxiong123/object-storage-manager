...
| 设计 token（`crates/ui/src/tokens.rs`） | 字号阶梯（`caption`/`label`/`body`/`heading`/`title`/`display`/`icon_sm`/`icon_lg`）、圆角（`radius`/`radius_lg`/`radius_icon`/`radius_nested`）、行距（`row_pad_y`/`row_pad_y_sidebar`/`row_pad_y_transfer`）与**随字号缩放**的列宽（`col_size_width`/`col_time_width`）都只在这里定义。新增文字/圆角/行距先选现有档位，不要新增第 9 个数字；任何跟随字号的尺寸都必须经 `text()`，固定 `px()` 在 140% 档位会与文字错位（列宽曾如此）。`radius_nested(inset)` = 外圆角 − 内缩（同心圆角规则），嵌套面紧贴时用它推导而不是复用 `radius()`。`theme.rs` 管颜色与组件级 `Theme.radius`，`tokens.rs` 管自绘元素的尺寸 |
