...
| ⌘F 过滤 | `filter_entries` 纯函数（大小写不敏感子串，命中 key/前缀名，单测锁死）；只影响展示不动 entries/选择集合（Finder 语义）。过滤条 context "ObjectFilter" 接 DismissFilter（Esc）；切桶即关闭，reload 后按新数据重算。跳转 Bucket：带数据 Action `SelectBucketByName(String)`（`#[action(no_json)]`），命令面板动态命令经 `PaletteCommand::handler` 分发，WorkspaceView 统一处理——handler 拿不到 Workspace 状态，禁止在闭包里直改 |
