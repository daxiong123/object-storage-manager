...
| 弹层规范（所有自建 overlay 统一） | **Esc 必关**：有输入框的经专属 context 绑定（`Renaming`→DismissRename、`AccountModal`/`SettingsModal`→DismissModal、`ObjectFilter`→DismissFilter），无输入框的（详情/预览）统一 context `Overlay` + `UnifiedDismiss`，卡片上 `on_action` 注册 handler；**遮罩点击关**：遮罩 `on_mouse_down` 调 close，busy 中由 close handler 自行拒绝（保存/创建/复制移动中不可关）；**卡片必须阻断冒泡**：一律 `on_mouse_down stop_propagation`，含滚动列表的（预览/复制移动）补 `on_mouse_up`（纯函数 `overlay_scroll_dismisses_modal` 锁死判据）；**视觉统一**：卡片圆角 8（= radius_lg，Linear 风格小圆角）、标题字号 16 SEMIBOLD、标题栏右侧关闭按钮（busy 时 disabled）、底部按钮「取消在左、主操作在右」（macOS HIG 顺序）。新弹层先按本行自查再实现 |
